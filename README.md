...
| **차트** | 토큰·비용 시계열. 이번 실행 / 7일 / 30일 / 90일 범위 전환 |
| **필터** | 속성(`user.email`·`unit`·`model` 등) 기준으로 스트림·통계·차트를 함께 걸러낸다 |
...
- **요청 원문을 저장하지 않는다.** 캡처는 메모리에만 있고(최대 500건) 앱을 끄면 사라진다.
  디스크에 남는 것은 날짜×계정별 **합계와 유출 기록**뿐이다(기본 90일 보관, 1년 써도 1MB 미만).
  원문이 필요하면 JSON/CSV 로 내보낸다.
