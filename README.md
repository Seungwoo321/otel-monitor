...
**Claude Code 가 내보내는 텔레메트리를 눈으로 확인하는 데스크톱 앱**
...
조직에서 AI 거버넌스 목적으로 Claude Code 의 OpenTelemetry 수집을 켜는 경우가 늘고 있다.
설정 파일에 환경변수 몇 개를 심으면 1분에 한 번 수집 서버로 메트릭이 나간다.

공식 문서상으로는 **메트릭(숫자 집계)만 나가고 프롬프트·코드 원문은 나가지 않는다.**
`OTEL_LOGS_EXPORTER` 가 없으면 이벤트 신호 자체가 열리지 않고, 내용 노출 스위치 5종도 전부 기본 off 다.

문제는 그게 **"지금 이 순간에도 그런지"** 확인할 방법이 없다는 것이었다.

- 전송 성공·실패 로그가 디스크에 남지 않는다 (문서에 로그 경로 자체가 없다)
- 실패는 `claude --debug` 의 `[3P telemetry]` 줄로만 드러난다 — 평소엔 조용히 실패한다
- 나중에 설정에 키가 추가돼 범위가 넓어져도 알아챌 방법이 없다

그래서 **중간에 세워두고 지나가는 걸 전부 뜯어보는** 도구를 만들었다.

## 어떻게 동작하나

투명 프록시다. 앱이 로컬에서 OTLP 를 받아 기록하고, 원래 수집 서버로 **바이트를 그대로** 전달한다.
수집은 정상적으로 유지되면서 내용만 들여다볼 수 있다.
...
이 앱을 만든 실질적인 이유다. 다음이 페이로드에 나타나면 상단에 경고 배너가 뜬다.
...
설정 탭의 진단 기능은 `settings.json` 을 직접 읽어 내용 노출 키 6종이 있는지 확인해 준다.
설정이 언제 바뀌든 앱을 켜두면 바로 드러난다.
...
숫자뿐이고 내용은 없다. 다만 **로그인 상태면 `user.email` 과 `organization.id` 가 함께 붙는다** —
익명 통계가 아니라 개인 단위로 식별되는 수집이라는 뜻이다. 인스펙터가 이 속성들을 따로 표시해 준다.
...
- **앱을 꺼두면 그 구간 텔레메트리가 수집 서버에 도달하지 않는다.** 프록시를 중간에 두는 이상
  피할 수 없는 트레이드오프이므로, 수집이 의무인 환경에서는 이 점을 감안해서 쓸 것.
