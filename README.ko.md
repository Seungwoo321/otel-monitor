...
| **설정** | 포트 · 전달 서버 · 전달 on/off, 기록 보관 기간, 내보내기, 업데이트 확인, 설정 파일 진단 |
| **메뉴바** | 창을 닫아도 메뉴바에 남아 계속 받는다. 우클릭하면 비용·토큰·세션·활동 요약 |
...
창을 닫아도 앱은 메뉴바에 남아 계속 받는다. 아이콘을 우클릭하면 그 시점 수치가 뜨고,
좌클릭하면 창이 열리고 닫힌다. 유출이나 전달 실패는 값이 있을 때만 줄이 붙는다.

### 스트림 자동 추적

`tail -f` 처럼 새 항목이 오면 자동으로 선택되고 인스펙터가 따라간다.
특정 항목을 클릭하면 멈추고, "최신으로" 를 누르면 다시 따라간다.
